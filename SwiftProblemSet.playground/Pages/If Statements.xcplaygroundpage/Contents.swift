...
if finishedWork == true && gotTickets {
goToConcert()
}
...
func checkForRelease(bugs: Int, music: Bool, levels: Int) {
    ...
    release()
    } else if bugs > 10 || music == false || numberOfLevels <= 5 {
        if bugs > 10 {
         print("bugs need to be fixed since no of bugs are- \(bugs)")
        }
        
        if music == false
        {
         print("game does not have a music. music needs to be added")
        }
        
        if numberOfLevels <= 5
        {
         print("game does not enough levels. levels are - \(levels)")
        }
    }
...
func checkTrainingStatus(name: String, bike: Bool, run: Bool) {
    //TODO: Add your if, elserunif statement here!
    if bike == true && run == true {
    print("\(name) is ready for race")
    } else if bike == false || run == false {
        if bike == false {
            print("\(name) is not ready for biking")
        }
       if run == false {
           print("\(name) is not ready for racing")
      }
    }
}
